...
def output_dryad_to_files(dataset):
    fn_doi = sanitize_filename(dataset['identifier'])

    directory_path = f"output/{fn_doi}"
    os.makedirs(directory_path, exist_ok=True)  # Ensure the directory exists

    with open(f"output/{fn_doi}/dryad_meta.json", 'w') as f:
    ...
    dc_json = datacite_json_metadata(dataset['identifier'])
    if dc_json:
